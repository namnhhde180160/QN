...
public class productDAO {
    ...
    public List<Product> getProduct() {
        ...
        String sql = """
                     select c.category_name , p.product_id , p.product_name, p.product_price, p.product_describe, p.quantity,p.img from  
    ...
    try (Connection conn = new DBContext().getConnection()) {
        ...
        String[] sqls = {
            "DELETE FROM bill_detail WHERE product_id = ?",
        ...
        conn.commit();
        System.out.println(">> Đã xóa sản phẩm thành công: " + product_id);
    } catch (Exception e) {
        e.printStackTrace();
    }
}
